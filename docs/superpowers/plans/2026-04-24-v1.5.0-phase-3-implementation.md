...
| Chunk | Goal | Files | Tests added | Time |
|---|---|---|---|---|
| 1 | Pre-flight + 7 Pydantic models | `src/td_mcp/models/patch.py` | ~13 | 60–90 min |
| 2 | UndoBlockSentinel + Validator | `src/td_mcp/patch/undo_sentinel.py`, `patch/validator.py`, `patch/__init__.py` | ~8 | 45–60 min |
| 3 | Variants + Planner | `src/td_mcp/patch/variants.py`, `patch/planner.py` | ~13 | 75–90 min |
| 4 | Applier | `src/td_mcp/patch/applier.py` | ~7 | 90–120 min |
| 5 | MCP tool wrappers | `src/td_mcp/registry/tools_patch.py`, schema snapshot regen | ~12 | 75–90 min |
| 6 | Legacy shim refactor + live smoke + acceptance | `src/td_mcp/registry/tools_planning.py`, `scripts/patch_session_smoke.py` | ~0 | 45–60 min |
| **Total** | 97 tools, ~648 tests | | **~52** | **6–10 hrs** |

---

## Chunk 1: Pre-flight + Pydantic models

**Goal:** Confirm the branch is ready for Phase 3 work, then land all 7 Pydantic models with per-model tests. After this chunk, `from td_mcp.models.patch import PatchPlan, PatchOperation, PatchResult, PatchPreview, PatchVariant, ValidationPlan, ValidationReport` works and ~13 new tests are green.
...
- [ ] **Step 4: Run all tests — expect all ~13 green**

Run: `uv run pytest tests/models/test_patch_models.py -v`

Expected: `13 passed` (5 PatchOperation + 2 ValidationPlan + 6 PatchPlan + 2 PatchPreview + 2 ValidationReport + 2 PatchResult + 3 PatchVariant = 22; adjust if a few test methods were added inline — aim ≥13).
...
```
Expected: `609 passed` (596 + ~13), ruff clean.
...
Expected: `609 tests collected` (or slightly higher).

- [ ] **Step 2: Mark chunk 1 complete in TodoWrite**
...
uv run ruff format src/ tests/
```
Expected: `617 passed` (609 + 5 sentinel + 3 validator), ruff clean.
...
uv run ruff format src/ tests/
```
Expected: `630 passed` (617 + 6 variants + 7 planner), ruff clean.
...
uv run ruff format src/ tests/
```
Expected: `637 passed` (630 + 7 applier), ruff clean.
...
Append to `src/td_mcp/registry/tools_patch.py`:
```python
from td_mcp.models.patch import PatchPreview
...
git commit -m "feat(tools): td_patch_preview + patch.preview_plan (live_risk_flags)"
```
...
Expected: tests pass, ruff clean, tool count 95.

```bash
git add src/td_mcp/registry/tools_patch.py tests/test_tools_patch.py
git commit -m "feat(tools): td_patch_apply — envelope + outcome split, sentinel DI"
```
...
- [ ] **Step 2: Add behavioural test + commit**

Append test, run suite, commit as:
```bash
git commit -m "feat(tools): td_patch_validate — composite validation report"
```
...
```bash
git commit -m "feat(tools): td_patch_variations — n-variant param_jitter (92→97)"
```
...
uv run ruff format src/ tests/
```
Expected: `~649 passed` (637 + ~12 MCP tool tests), ruff clean.
...
```python
def _legacy_plan_dict(plan, *, intent: str, target_path: str, recipe_id: str | None) -> dict[str, Any]:
    """Translate a typed PatchPlan into the pre-v1.5.0 dict shape so
    legacy callers of td_plan_patch see no change.
    """
    ...
    dict_out: dict[str, Any] = {
        ...
        "recipe_id": recipe_id,
        "current_node_count": 0,                  # legacy field; recomputed if needed
        "existing_names": [],                      # legacy field
        "steps": steps,
        "note": (
...
uv run ruff format --check src/ tests/
```
Expected: `~648 passed`, ruff clean.
...
```
Expected: `~648 passed` (or higher — depends on exact test counts).
...
Report:
> "Phase 3 Patch Session MVP complete on branch `v1.5.0/module-splits`. Tool count 92→97, tests ~648, ruff clean. Live smoke script passed (if TD was available). Ready for Phase 4 (Creative Recipe Pack) or v1.5.0-rc tagging via @superpowers:finishing-a-development-branch."
