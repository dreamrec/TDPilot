...
> TDPilot v2.4.0 | 114 tools | This region is **generated** from the FastMCP registry by `scripts/gen_api_reference.py` — do not edit it by hand. Regenerate with `uv run python scripts/gen_api_reference.py`; CI enforces freshness with `--check`. Sections group tools by their `src/td_mcp/registry/` module of origin.
