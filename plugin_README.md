...
TDPilot v1.6.8 provides 103 MCP tools for live control of TouchDesigner projects from Claude (plus the v1.6.1 expanded hint corpus — 19 packs / 63 hints — with v1.6.2 surface routing for response-context-aware hints), plus a one-button-install panel inside the `.tox` itself (drag-drop into TD, click "Bootstrap All", done). v1.6.8 fixes the last v1.5.6-line build-script regression: `status_text.nodeX = 600` placed the panel TOP outside the 520×320 panel viewport, so it cooked correctly as a TOP but never composited into the panel surface. Now positioned at `(0, 0)` inside the viewport. Combined with the v1.6.7 fixes (state_cache restored, autostart triggers enabled, display flag set, externaltox param-name corrected), the panel renders correctly out-of-the-box on every fresh `.tox` load.
