...
TDPilot v1.6.6 provides 103 MCP tools for live control of TouchDesigner projects from Claude (plus the v1.6.1 expanded hint corpus — 19 packs / 63 hints — with v1.6.2 surface routing for response-context-aware hints), plus a one-button-install panel inside the `.tox` itself (drag-drop into TD, click "Bootstrap All", done). v1.6.6 makes "Update Now" actually refresh the loaded COMP — the autostart's `save_toe` handler now sets `externaltox` on the COMP before saving, so future TD launches read the latest .tox content fresh from disk instead of restoring a frozen embedded copy. Closes the "panel says X but disk has Y" drift class permanently (which v1.6.5's Startup-script sweep alone couldn't, because Startup scripts run BEFORE the .toe loads).
