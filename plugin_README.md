...
TDPilot v1.6.9 provides 103 MCP tools for live control of TouchDesigner projects from Claude (plus the v1.6.1 expanded hint corpus — 19 packs / 63 hints — with v1.6.2 surface routing for response-context-aware hints), plus a one-button-install panel inside the `.tox` itself (drag-drop into TD, click "Bootstrap All", done). v1.6.9 finally closes the v1.6.x panel-rendering saga: the containerCOMP's Look-page `top` parameter (panel-background TOP reference) was never wired by the v1.5.6 build script — the actual root cause behind 6 prior wrong-layer fixes. Now `comp.par.top = status_text` is set at build time, plus the textTOP renders at native panel resolution (520×320, no horizontal stretch) with cyan-greenish text on 90%-opaque black for clean visibility.
