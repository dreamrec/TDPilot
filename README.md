...
[![MCP tools](https://img.shields.io/badge/MCP%20tools-99-blueviolet)](./docs/API_REFERENCE.md)
...
That installs all **99 MCP tools**, 3 skills (`tdpilot-core`, `tdpilot-production`, `popx-touchdesigner`), 2 slash commands (`/td-check`, `/td-snapshot`), and the TD-side `.tox` component — one command, no Python setup required.
...
- 99-tool runtime surface with knowledge corpus, vision diagnostics, TD 2025 native inspection, official recommendations, job resources, memory, optimizer, safety, POPx inspection, project lifecycle control, custom parameter authoring, and typed patch sessions.

## Start Here: Core Workflow

You don't need all 99 tools. Start with these and expand as needed:
