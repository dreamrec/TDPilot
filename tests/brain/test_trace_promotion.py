...
from td_mcp.models.brain import BrainPlan, BrainTrace, ConceptGraph, VisualTaskSpec
...
def test_successful_brain_trace_promotes_to_docs_grounded_pattern_candidate():
    ...
    assert {"audiofileinCHOP", "feedbackTOP", "panelCHOP", "textDAT"}.issubset(
        set(pattern.required_ops)
    )
...
def test_promoted_trace_pattern_becomes_ranked_resolver_candidate():
    ...
    assert candidates[0].score > next(
        ...
        if "audio_file_to_analysis_chop" in candidate.pattern_ids
    )
