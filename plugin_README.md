...
TDPilot v1.6.7 provides 103 MCP tools for live control of TouchDesigner projects from Claude (plus the v1.6.1 expanded hint corpus — 19 packs / 63 hints — with v1.6.2 surface routing for response-context-aware hints), plus a one-button-install panel inside the `.tox` itself (drag-drop into TD, click "Bootstrap All", done). v1.6.7 fixes three build-script regressions present since v1.5.6 that caused fresh `.tox` loads to render an empty "Ctn" panel placeholder: the `state_cache` DAT (renderer's data source) was never created, the `autostart` executeDAT's trigger toggles were all left at their default `False` (so `onStart`/`onFrameStart` never fired), and `status_text.display` was never enabled (so the panel TOP never surfaced in the COMP panel). Plus fixes v1.6.6's wrong externaltox parameter name (`reloadtoxonstart` → `enableexternaltox`).
